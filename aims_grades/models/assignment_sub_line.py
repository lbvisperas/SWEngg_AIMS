...
class StudentSectionSubLine(models.Model):
    _name = "student.list.grades"
    _inherit = "mail.thread"
    _rec_name = "assignment_id"
    _description = "Student Submission"
    ...
    def get_user_group(self):
        for user in self:
            if self.env.user.has_group(
                    'aims_student_academic.group_student_back_office_admin') or \
                    self.env.user.has_group(
                        'aims_student_academic.group_student_back_office') or \
                    self.env.user.has_group(
                        'aims_student_academic.group_student_faculty'):
                user.user_boolean = True
            else:
                user.user_boolean = False

    assignment_id = fields.Many2one('student.section', 'Section', required=True)
    ...
    def unlink(self):
        ...
        res = super(StudentSectionSubLine, self).unlink()
        return res
    ...
    def create(self, vals):
        ...
        return super(StudentSectionSubLine, self).create(vals)
    ...
    def write(self, vals):
        ...
        return super(StudentSectionSubLine, self).write(vals)
